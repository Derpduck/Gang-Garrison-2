...
while(nearestEnemy == -1 && !ds_priority_empty(targetQueue)) {
    ...
    if (playercheck.team != team
        && playercheck.ubered==0
        && collision_line(botPlayer.object.x,botPlayer.object.y,playercheck.x,playercheck.y,Obstacle,true,true)<0
        //&& collision_line(object.x,object.y,playercheck.x,playercheck.y,TeamGate,true,true)<0
        && collision_line(botPlayer.object.x,botPlayer.object.y,playercheck.x,playercheck.y,ControlPointSetupGate,true,true)<0
        && collision_line(botPlayer.object.x,botPlayer.object.y,playercheck.x,playercheck.y,IntelGate,true,true)<0
        && collision_line(botPlayer.object.x,botPlayer.object.y,playercheck.x,playercheck.y,BulletWall,true,true)<0){
