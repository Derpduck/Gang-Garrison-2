...
for (Y=0; Y<map_height()-1; Y+=1){
    // Generate a node in all possible floor pixels
...
for(i=0; i<size; i+=1){
    // Get rid of all those that are in a horizontal row, save the edges.
    if !inRow{
        ...
        if positions[i, 0]+1 != positions[i+1, 0] or positions[i, 1] != positions[i+1, 1]{
...
for(i=0; i<ds_list_size(nodeList); i+=1){
    // Get rid of long stairs, save the edges (again). Also get rid of any in a spawnroom
    ...
    while newNode{
        ...
        if newNode{
            if !instance_position(newNode.x-6, newNode.y+6, Node){
                // If the next node is the last one, spare it
                break;
            }
        }
    }
    ...
    while newNode{
        ...
        if newNode{
            if !instance_position(newNode.x-12, newNode.y+6, Node){
                // If the next node is the last one, spare it
                break;
            }
        }
...
for(i=0; i<ds_list_size(nodeList); i+=1){
    // Remove nodes almost directly underneath another node.
    node = ds_list_find_value(nodeList, i);
