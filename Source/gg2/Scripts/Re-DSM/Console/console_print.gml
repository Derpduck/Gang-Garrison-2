...
while string_length(input) > global.consoleOutputMaxLength{
    var position, tempString, oldPosition, prefix;
 
    position = 0
    oldPosition = 0
    tempString = string_copy(input, 0, global.consoleOutputMaxLength-1)
    ...
    if string_count(" ", tempString) != 0{// Break the line at spaces if possible
        ...
        ds_list_add(global.consoleOutputHistory, string_copy(input, 0, oldPosition));
        ...
        ds_list_add(global.consoleOutputHistory, string_copy(input, 0, global.consoleOutputMaxLength-1));
        prefix = string_copy(input, 0, 3 + COLOR_RGB_LENGTH)
        input = string_copy(input, global.consoleOutputMaxLength, string_length(input));
...
while ds_list_size(global.consoleOutputHistory) > global.consoleLines{
